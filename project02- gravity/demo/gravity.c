...
*/
void add_star(array_list_t *star_arr, star_t *star){
    array_list_add(star_arr, star);
}
...
bool reaches_bottom(star_t *star){
    int num_vertices = array_list_size(star_get_vertices(star));
    for (int i = 0; i < num_vertices; i++){
        double y = ((vector_t*)array_list_get(star_get_vertices(star), i))->y;
 ...
 * If it is out of bounds, returns true. Else, returns false.
 */
bool star_out_of_bounds(star_t *star){
    int num_vertices = array_list_size(star_get_vertices(star));
    for (int i = 0; i < num_vertices; i++){
        double x = ((vector_t*)array_list_get(star_get_vertices(star), i))->x;
        if (x <= MAX.x){
            return false;
        }
    }
...
*/
void remove_out_of_bounds_stars(array_list_t *star_arr){
   star_t *curr = array_list_get(star_arr, 0);
   if (star_out_of_bounds(curr)){
        array_list_remove(star_arr);
    }
}
 ...
 * @param dt time since last update
*/
void compute_new_positions(array_list_t *star_arr, double dt){
    int size = array_list_size(star_arr);
    for (int i = 0; i < size; i++){
        star_t *curr = array_list_get(star_arr, i);
 ...
 * @param star_arr array of stars
*/
void draw_stars(array_list_t *star_arr){
    int size = array_list_size(star_arr);
    for (int i = 0; i < size; i++){
        star_t *curr = array_list_get(star_arr, i);
        sdl_draw_polygon((array_list_t*)star_get_vertices(curr), star_get_color(curr).r , star_get_color(curr).g, star_get_color(curr).b);
    }
}
 ...
 * @param star_arr array of stars
*/
void update_window(array_list_t *star_arr){
    sdl_clear();
    draw_stars(star_arr);
    sdl_show();
}
...
int main(){
    array_list_t *star_arr = array_list_init(NUM_STARS); 
    double clock = 0;
