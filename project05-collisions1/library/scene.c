...
bool force_aux_is_removed(force_aux_t *fa) {
...
void scene_tick(scene_t *scene, double dt) {
    ...
    for (int i = list_size(fas) - 1; i >= 0; i--) {
        fa_curr = list_get(fas, i);
        if (force_aux_is_removed(fa_curr)) {
            force_aux_free(list_remove(fas, i));
        }
    }
...
void scene_add_bodies_force_creator(scene_t *scene,
                                    ...
                                    list_t *bodies,
                                    free_func_t freer) {
    list_add(scene->force_auxs, force_aux_init(forcer, freer, aux, bodies));
}
