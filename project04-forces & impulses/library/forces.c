...
// The minimum speed to compute drag forces at.
const double DRAG_MIN_VELOCITY_THRESHOLD = 0;

/**
 * Private struct to keep track of parameters to be given to the force creator
 * functions.
 */
typedef struct aux {
    list_t *bodies;
...
void force_creator_drag(aux_t *aux) {
    ...
    if (vec_magnitude(v) > DRAG_MIN_VELOCITY_THRESHOLD) {
