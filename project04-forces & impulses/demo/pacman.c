...
const rgb_color_t PACMAN_COLOR[3] = {1.0, 0.64, 0.0};
#define TOTAL_COLORS 9
const rgb_color_t PELLET_COLORS[TOTAL_COLORS][3] = {
...
// bool pacman_should_remove(body_t * curr_shape);
...
/**
 * pacman_should_remove should only be called immediately following
 * pacman_collision on the same shape. If the curr_shape has collided
 * with another object, this tells world whether or not to remove it.
 * @param curr_shape the current shape that has collided with something
 * @return bool, TRUE if shape is pellet, FALSE if shape is pacman
 *
bool pacman_should_remove(scene_t * world, body_t * curr_shape) {
    // TODO fix this
    return (curr_shape != (body_get_shape(scene_get_body(world, 0)));
}
*/
...
void pacman_wrap(body_t *pacman, int min_x, int min_y, int max_x, int max_y) {
    ...
    for (int i = 0; i < PACMAN_RESOLUTION + 1; i++) {
        vector_t *curr_point = list_get(body_get_shape(pacman), i);
...
int main(int argc, char *argv[]) {
    ...
    sdl_on_key(pacman_on_key, pacman);
