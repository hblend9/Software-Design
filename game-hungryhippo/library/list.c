...
list_t *list_init_from_path(const char *file_path,
    ...
    list_t *l = list_init(1, freer);

    char *curr_line = malloc(sizeof(char) * (MAX_LINE_LENGTH + 1));

    while (fgets(curr_line, MAX_LINE_LENGTH, fp) != NULL) {
        list_add(l, parser(curr_line));
    }
...
void list_resize_if_needed(list_t *list) {
    if (list_size(list) >= list->capacity) {
        size_t new_capacity = (size_t)(list->capacity * LIST_RESIZE_FACTOR) + 1;
...
void list_reverse(list_t *list) {
    for (size_t i = 0; i < list->size; i++) {
        list_add(list, list_remove(list, list->size - 1 - i));
    }
}
