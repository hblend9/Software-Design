...
void add_gravity_body(scene_t *scene) {
    ...
    body_t *body = body_init_with_info(gravity_ball,
                                       M,
                                       WALL_COLOR,
...
body_t *get_ball(vector_t center, vector_t velocity) {
    ...
    body_t *ball = body_init_with_info(shape,
                                       BALL_MASS,
                                       BALL_COLOR,
                                       make_type_info(BALL),
                                       free);
...
void add_ball(scene_t *scene) {
    ...
    vector_t ball_center
        = {.x = MAX.x / 2 + (rand_double() - 0.5) * DELTA_X, .y = DROP_Y};
...
void add_pegs(scene_t *scene) {
    ...
    for (size_t i = 1; i <= N_ROWS; i++) {
        for (size_t j = 0; j <= i; j++) {
            ...
            body_t *body = body_init_with_info(polygon,
                                               INFINITY,
                                               PEG_COLOR,
                                               make_type_info(WALL),
                                               free);
...
void add_walls(scene_t *scene) {
    ...
    body_t *body = body_init_with_info(rect,
                                       INFINITY,
                                       WALL_COLOR,
                                       make_type_info(WALL),
                                       free);
