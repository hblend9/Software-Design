...
const size_t ENEMY_RESOLUTION = 20;
...
void si_force_creator_enemy_wall_collision(aux_t *aux) {
    ...
    if (find_collision(enemy_shape, b_wall_shape).collided) {
        body_remove(list_get(aux->bodies, 4));
    } else if (find_collision(enemy_shape, l_wall_shape).collided) {
        // move the enemy down
        body_translate(enemy,
                       vec_multiply((-NUM_ROWS) * (ENEMY_SIZE + BUFFER), e2));
        // move the enemy away from the edge
        vector_t reset_x = {.x = MIN_X + ENEMY_SIZE / 2.0 + 1,
                            .y = body_get_centroid(enemy).y};
    ...
    } else if (find_collision(enemy_shape, r_wall_shape).collided) {
        // move the enemy down
        body_translate(enemy,
                       vec_multiply((-NUM_ROWS) * (ENEMY_SIZE + BUFFER), e2));
        // move the enemy away from the edge
        vector_t reset_x = {.x = MAX_X - ENEMY_SIZE / 2.0 - 1,
                            .y = body_get_centroid(enemy).y};
        body_set_centroid(enemy, reset_x);
        // switch directions of the enemy's movement
        body_set_velocity(enemy, vec_multiply(-1, body_get_velocity(enemy)));
...
void si_force_creator_player_wall_collision(aux_t *aux) {
    ...
    if (find_collision(player_shape, l_wall_shape).collided) {
        body_translate(player, e1);
        body_set_velocity(player, VEC_ZERO);

    } else if (find_collision(player_shape, r_wall_shape).collided) {
...
void si_force_creator_laser_wall_collision(aux_t *aux) {
    ...
    if (find_collision(laser_shape, wall_shape).collided) {
        body_remove(laser);
    }
